...
{
	$sql	= ($_POST["id"] > 0) ?
		"UPDATE `magic_decks` SET `account_id` = ". $_SESSION["id"] .", `title` = \"". addslashes($_POST["deckname"]) ."\", `cards` = \"". addslashes($_POST["deck"]) ."\" WHERE `id` = ". $_POST["id"] .";":
		"INSERT INTO `magic_decks` (`account_id`, `title`, `cards`) VALUES (". $_SESSION["id"] .", \"". addslashes($_POST["deckname"]) ."\", \"". addslashes($_POST["deck"]) ."\");";
	query($sql);
}

if (isset($_POST["delete"]) && $_POST["id"] > 0)
{
	if (query("SELECT * FROM `magic_decks` WHERE `id` = ". $_POST["id"] ." AND `account_id` = ". $_SESSION["id"]))
		query("DELETE FROM `magic_decks` WHERE `id` = ". $_POST["id"]);
}
